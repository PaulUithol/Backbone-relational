...
QUnit.module('Backbone.Relational.HasOne', {
	beforeEach() {
		reset();
		store.addModelScope({
	...
	QUnit.test('\'set\' triggers \'change\' and \'update\', on a HasOne relation, for a Model with multiple relations', function(assert) {
		...
		let password = new Password({ plaintext: 'asdf' });
		...
		let user = { login: 'me@hotmail.com', password: { plaintext: 'qwerty' } };
		let oldLogin = objects.person1.get('user').get('login');
	...
	QUnit.test('\'set\' doesn\'t triggers \'change\' and \'change:\' when passed `silent: true`', function(assert) {
		...
		let user = new User({ login: 'me@hotmail.com', password: { plaintext: 'qwerty' } });
