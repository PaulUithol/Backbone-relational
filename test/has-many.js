...
import { store, Model, HasMany, Collection } from 'backbone-relational';
...
QUnit.module('HasMany', {
	...
	QUnit.test('Listeners for \'add\'/\'remove\', on a HasMany relation, for a Model with multiple relations', function(assert) {
		let job1 = { company: objects.oldCompany };
		let job2 = { company: objects.oldCompany, person: objects.person1 };
		let job3 = { person: objects.person1 };
		let newJob = null;
		...
		let jobs = objects.person1.get('jobs');
		...
		let employees = objects.oldCompany.get('employees');
	...
	QUnit.test('The Collections used for HasMany relations are re-used if possible', function(assert) {
		let collId = objects.ourHouse.get('occupants').id = 1;
	...
	QUnit.test('On `set`, or creation, accept a collection or an array of ids/objects/models', function(assert) {
		...
		let visitor1 = new Visitor({ id: 'visitor-1', name: 'Mr. Pink' }),
		...
		let visitor3 = new Visitor({ id: 'visitor-3' });
		...
		// Handle a Collection
		let visitorColl = new Collection([visitor1, visitor2]);
	...
	QUnit.test('On `set`, or creation, handle edge-cases where the server supplies a single object/id', function(assert) {
		...
		let lion3 = new Animal({ id: 'lion-3' });
		zoo = new Zoo({
			animals: lion3
		});
		...
		let lion4 = new Animal({ id: 'lion-4' });
		assert.equal(animals.length, 1, 'There is 1 animal in the zoo');

		zoo.set('animals', 'lion-5');
		assert.equal(animals.length, 0, 'No animals in the zoo');

		let lion5 = new Animal({ id: 'lion-5' });
		assert.equal(animals.length, 1, 'There is 1 animal in the zoo');
	...
	QUnit.test('Setting a new collection maintains that collection\'s current \'models\'', function(assert) {
		...
		let newAnimals = new AnimalCollection([
			{ id: 2, species: 'Zebra' },
	...
	QUnit.test('The \'collectionKey\' options is used to create references on generated Collections back to its RelationalModel', function(assert) {
		...
		let FarmAnimal = Model.extend();
		let Barn = Model.extend({
			relations: [{
				type: HasMany,
				key: 'animals',
	...
	QUnit.test('Polymorhpic relations', function(assert) {
		let Location = Model.extend();

		let Locatable = Model.extend({
			relations: [
				{
					key: 'locations',
					type: 'HasMany',
		...
		let FirstLocatable = Locatable.extend();
		let SecondLocatable = Locatable.extend();

		let firstLocatable = new FirstLocatable();
		let secondLocatable = new SecondLocatable();

		let firstLocation = new Location({ id: 1, locatable: firstLocatable });
		let secondLocation = new Location({ id: 2, locatable: secondLocatable });
